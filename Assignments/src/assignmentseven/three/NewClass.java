package assignmentseven.three;

public class NewClass extends ConcreteClass implements FourthInterface{
    @Override
    public void method7() {
        System.out.println("Method7");
    }

    @Override
    public void method1() { System.out.println("Method1");}

    @Override
    public void method2() {System.out.println("Method2");System.out.println("Method3"); }

    @Override
    public void method3() { System.out.println("Method3");}

    @Override
    public void method4() { System.out.println("Method4");}

    @Override
    public void method5() { System.out.println("Method5");}

    @Override
    public void method6() {System.out.println("Method6"); }
}
